/**
 * dark.js ‘«ˆ Template Dark Forno
 * Est+Ætica noturna premium: grain, gradiente ember,
...
function buildHTML() {
  return `
  <div class="dk-topbar">≠Éˆ— Pedidos abertos por mais <span id="dk-countdown">--:--:--</span> ≠Éˆ—</div>

  <section class="dk-hero">
    <div class="dk-hero-bg"></div>
    <div class="dk-orbit">≠ÉÏÚ</div>
    <div class="dk-hero-inner">
      ...
      <div class="dk-stats">
        <div class="dk-stat"><span class="dk-stat-icon">‘≈¶</span><span class="dk-stat-label">Entrega</span><span class="dk-stat-val" id="dk-time">40‘«Ù60 min</span></div>
        <div class="dk-stat"><span class="dk-stat-icon">≠É¯¡</span><span class="dk-stat-label">Frete</span><span class="dk-stat-val" id="dk-fee">R$5</span></div>
        <div class="dk-stat"><span class="dk-stat-icon">‘°…</span><span class="dk-stat-label">Avalia+∫+˙o</span><span class="dk-stat-val" id="dk-rating">4.9</span></div>
        <div class="dk-stat"><span class="dk-stat-icon">≠ÉÚ…</span><span class="dk-stat-label">Hoje</span><span class="dk-stat-val" id="dk-hours">18h‘«Ù23h</span></div>
      </div>
      <button class="dk-cta" onclick="document.getElementById('dk-nav').scrollIntoView({behavior:'smooth'})">≠ÉÏÚ Ver Card+Ìpio</button>
    </div>
  </section>
  ...
  <nav class="dk-nav" id="dk-nav">
    <div class="dk-nav-inner">
      ...
      <button class="dk-cart-btn" onclick="dkToggleCart()">≠É¯∆<span class="dk-cart-badge" id="dk-cart-badge">0</span></button>
    </div>
  </nav>
  ...
  <main class="dk-main">
    <div id="dk-social" class="dk-social dk-fi-anim">
      ...
      <div class="dk-social-txt"><strong id="dk-social-txt">12 pessoas vendo agora</strong><span id="dk-social-sub">+‹ltima venda h+Ì 8 min</span></div>
      <span class="dk-social-live">≠Éˆ¶ Ao vivo</span>
    </div>
    <div id="dk-promo" class="dk-promo dk-fi-anim" style="display:none">
      <div><strong id="dk-promo-txt"></strong><div class="dk-promo-sub">Promo+∫+˙o por tempo limitado</div></div>
      <div class="dk-promo-pill" id="dk-promo-tag">S+Ù HOJE</div>
    </div>
  ...
  <div class="dk-imbg" id="dk-imbg" onclick="dkCloseIM(event)">
    <div class="dk-imbox">
      <div class="dk-imimg">
        ...
        <div class="dk-imnoimg" id="dk-im-noimg">≠ÉÏÚ</div>
        <button class="dk-imclose" onclick="document.getElementById('dk-imbg').classList.remove('on')">‘£Ú</button>
      </div>
      <div class="dk-imbody">
  ...
  <div class="dk-cdr" id="dk-cdr">
    <div class="dk-chd"><div class="dk-chd-t">≠É¯∆ Seu Pedido</div><button class="dk-cx" onclick="dkToggleCart()">‘£Ú</button></div>
    <div class="dk-cbody" id="dk-cbody"><div class="dk-cempty">Carrinho vazio ≠ÉÏÚ</div></div>
    <div class="dk-cft" id="dk-cft" style="display:none">
      ...
      <div class="dk-cwarn" id="dk-cwarn" style="display:none">‘‹·¥©≈ M+°nimo R$<span id="dk-cwarn-min">30</span>. Faltam R$<span id="dk-cwarn-diff">30</span></div>
      <input class="dk-fi" id="dk-cname" placeholder="Seu nome">
      <input class="dk-fi" id="dk-caddr" placeholder="Endere+∫o de entrega">
      <button class="dk-obtn" onclick="dkCheckout()">Fazer Pedido ≠ÉÏÚ</button>
    </div>
  </div>
...
function populateHero() {
  ...
  $('dk-time').textContent    = store.deliveryTime || '40‘«Ù60 min';
  $('dk-fee').textContent     = store.fee ? `R$${store.fee}` : 'R$5';
  $('dk-rating').textContent  = store.rating || '4.9 ‘ˇ‡';
  ...
  $('dk-foot-phone').textContent = store.phone ? `≠ÉÙ◊ ${store.phone}` : '';
  const logo = $('dk-logo');
  if (store.logo) { logo.src = store.logo; logo.classList.add('show'); }
  if (store.promoTxt) {
    $('dk-promo-txt').textContent = store.promoTxt;
    $('dk-promo-tag').textContent = store.promoTag || 'S+Ù HOJE';
...
function dkCard(p, cat) {
  return `<div class="dk-card" onclick="dkOpenIM('${p.id}')">
    <div class="dk-card-img">${p.img ? `<img src="${p.img}" alt="${p.name}">` : '≠ÉÏÚ'}</div>
...
function dkListItem(p) {
  return `<div class="dk-list-item" onclick="dkOpenIM('${p.id}')">
    <div class="dk-list-img">${p.img ? `<img src="${p.img}" alt="">` : '≠ÉÏÚ'}</div>
    <div style="flex:1">
...
function renderCombos() {
  ...
  sec.innerHTML = `<div class="dk-sec-head"><h2 class="dk-sec-title">≠Éƒ¸ <em>Combos</em> Especiais</h2><span class="dk-sec-count">${combos.length} op+∫+¡es</span></div>
  <div class="dk-combos-scroll">${combos.filter((c) => c.active !== false).map((c) => `
    <div class="dk-combo-card" onclick="dkAddCombo('${c.id}')">
      <div class="dk-combo-img">${c.img ? `<img src="${c.img}" alt="">` : '≠Éƒ¸'}${c.saving ? `<div class="dk-combo-save">${c.saving}</div>` : ''}</div>
...
function renderReviews() {
  ...
  sec.innerHTML = `<div class="dk-sec-head"><h2 class="dk-sec-title">‘°… <em>Avalia+∫+¡es</em></h2></div>
  <div class="dk-reviews-avg"><span class="dk-score">${avg}</span><div><div style="color:var(--dk-gold)">${'‘°…'.repeat(5)}</div><div style="font-size:.72rem;color:var(--dk-muted)">${reviews.length} avalia+∫+¡es</div></div></div>
  <div class="dk-reviews-grid">${reviews.map((r) => `
    <div class="dk-review-card">
      <div class="dk-rc-top">
        <div class="dk-rc-av">≠ÉÊÒ</div>
        <div><div class="dk-rc-name">${r.name}</div><div class="dk-rc-stars">${'‘°…'.repeat(r.stars || 5)}</div></div>
      </div>
      <div class="dk-rc-text">${r.text}</div>
      ${r.product ? `<div class="dk-rc-product">≠ÉÙ™ ${r.product}</div>` : ''}
    </div>`).join('')}
  </div>`;
}
...
function renderCart() {
  ...
  if (!count) { $('dk-cbody').innerHTML = '<div class="dk-cempty">Carrinho vazio ≠ÉÏÚ</div>'; $('dk-cft').style.display = 'none'; return; }
  const sub = cart.reduce((s, i) => s + i.price * (i.qty || 1), 0);
  const fee = store.fee ?? 5; const total = sub + fee;
  const min = store.minOrder ?? 30;
  $('dk-cbody').innerHTML = cart.map((item) => `
    <div class="dk-ci">
      <div class="dk-ci-img">${item.img ? `<img src="${item.img}" alt="">` : '≠ÉÏÚ'}</div>
      <div style="flex:1">
        ...
        <div class="dk-ci-ctrl">
          <button class="dk-qb" onclick="dkChangeQty(${item.id},-1)">‘Í∆</button>
...
window.dkCheckout = function () {
  ...
  const fee = store.fee ?? 5; const total = sub + fee;
  const lines = cart.map((i) => `‘«Û ${i.name}${i.size ? ' (' + i.size + ')' : ''} ‘«ˆ R$${(i.price*(i.qty||1)).toFixed(2)}`).join('\n');
...
function startSocial() {
  ...
  $('dk-social-sub').textContent = `+‹ltima venda h+Ì ${Math.floor(Math.random()*15+1)} minutos`;
}
