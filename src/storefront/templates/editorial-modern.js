/**
 * editorial.js ‘«ˆ Template Editorial
 * Layout claro e clean: hero persuasivo, busca integrada,
 * barra de frete gr+Ìtis, stepper de quantidade e carrinho drawer.
 */
...
function buildHTML() {
  return `
  <div class="ed-topbar" id="ed-topbar">≠Éˆ— Pedidos abertos ‘«Û <span id="ed-countdown">--:--:--</span></div>
  <header class="ed-hero">
    <div class="ed-hero-inner">
      ...
      <div class="ed-stats">
        <div class="ed-stat">‘≈¶ <strong id="ed-time">40‘«Ù60 min</strong></div>
        <div class="ed-stat">≠É¯¡ <strong id="ed-fee">Frete R$5</strong></div>
        <div class="ed-stat">‘°… <strong id="ed-rating">4.9</strong></div>
        <div class="ed-stat">≠ÉÚ… <strong id="ed-hours">18h‘«Ù23h</strong></div>
      </div>
      <div class="ed-free-bar" id="ed-free-bar">
        ≠ÉƒÎ Frete gr+Ìtis acima de R$ <span id="ed-free-threshold">50</span>
  ...
  <nav class="ed-nav">
    <div class="ed-nav-inner">
      ...
      <div class="ed-search">
        <span class="ed-search-icon">≠ÉˆÏ</span>
        <input class="ed-search-inp" id="ed-search" type="text" placeholder="Buscar no card+Ìpio‘«™" oninput="onSearch(this.value)">
      </div>
      <div class="ed-cats" id="ed-cats"></div>
      <button class="ed-cart-btn" onclick="edToggleCart()">≠É¯∆<span class="ed-cart-badge" id="ed-cart-badge">0</span></button>
    </div>
  </nav>
  ...
  <main class="ed-main">
    <div id="ed-social" class="ed-social ed-fi-anim">
      ...
      <div class="ed-social-txt"><strong id="ed-social-txt">12 pessoas vendo agora</strong><span id="ed-social-sub">+‹ltima venda h+Ì 8 min</span></div>
      <span class="ed-social-live">Ao vivo</span>
    </div>
    <div id="ed-promo" class="ed-promo ed-fi-anim" style="display:none">
      <div class="ed-promo-text"><strong id="ed-promo-txt"></strong><span>Promo+∫+˙o por tempo limitado</span></div>
      <div class="ed-promo-pill" id="ed-promo-tag">S+Ù HOJE</div>
    </div>
  ...
  <div class="ed-imbg" id="ed-imbg" onclick="edCloseIM(event)">
    <div class="ed-imbox">
      <div class="ed-imimg" id="ed-imimg">
        ...
        <div class="ed-imnoimg" id="ed-im-noimg">≠ÉÏÚ</div>
        <button class="ed-imclose" onclick="document.getElementById('ed-imbg').classList.remove('on')">‘£Ú</button>
      </div>
      <div class="ed-imbody">
        ...
        <div class="ed-qty-stepper">
          <button class="ed-qty-btn" onclick="edChangeImQty(-1)">‘Í∆</button>
  ...
  <div class="ed-cdr" id="ed-cdr">
    <div class="ed-chd"><div class="ed-chd-t">≠É¯∆ Seu Pedido</div><button class="ed-cx" onclick="edToggleCart()">‘£Ú</button></div>
    <div class="ed-cbody" id="ed-cbody"><div class="ed-cempty">Carrinho vazio ≠ÉÏÚ</div></div>
    <div class="ed-cft" id="ed-cft" style="display:none">
      ...
      <div class="ed-cwarn" id="ed-cwarn" style="display:none">‘‹·¥©≈ M+°nimo R$<span id="ed-cwarn-min">30</span>. Faltam R$<span id="ed-cwarn-diff">30</span></div>
      <input class="ed-fi" id="ed-cname" placeholder="Seu nome">
      <input class="ed-fi" id="ed-caddr" placeholder="Endere+∫o de entrega">
      <button class="ed-obtn" onclick="edCheckout()">Fazer Pedido ≠ÉÏÚ</button>
    </div>
  </div>
...
function populateHero() {
  ...
  if (store.promoTxt) {
    ...
    $('ed-promo-tag').textContent = store.promoTag || 'S+Ù HOJE';
...
function edCard(p, cat) {
  return `<div class="ed-card" onclick="edOpenIM('${p.id}')">
    <div class="ed-card-img">${p.img ? `<img src="${p.img}" alt="${p.name}">` : '≠ÉÏÚ'}</div>
...
function edListItem(p) {
  return `<div class="ed-list-item" onclick="edOpenIM('${p.id}')">
    <div class="ed-list-img">${p.img ? `<img src="${p.img}" alt="">` : '≠ÉÏÚ'}</div>
...
function renderCombos() {
  ...
  sec.innerHTML = `<div class="ed-section-head"><h2 class="ed-section-title">≠Éƒ¸ Combos Especiais</h2><span class="ed-section-count">${combos.length} op+∫+¡es</span></div>
  <div class="ed-combos-scroll">${combos.filter((c) => c.active !== false).map((c) => `
    <div class="ed-combo-card" onclick="edAddCombo('${c.id}')">
      <div class="ed-combo-img">${c.img ? `<img src="${c.img}" alt="">` : '≠Éƒ¸'}${c.saving ? `<div class="ed-combo-save">${c.saving}</div>` : ''}</div>
...
function renderReviews() {
  ...
  sec.innerHTML = `<div class="ed-section-head"><h2 class="ed-section-title">‘°… Avalia+∫+¡es</h2></div>
  <div class="ed-reviews-grid">${reviews.map((r) => `
    <div class="ed-review-card">
      <div class="ed-review-stars">${'‘ˇ‡'.repeat(r.stars || 5)}</div>
...
function renderCart() {
  ...
  if (!count) { $('ed-cbody').innerHTML = '<div class="ed-cempty">Carrinho vazio ≠ÉÏÚ</div>'; $('ed-cft').style.display = 'none'; return; }
  const sub = cart.reduce((s, i) => s + i.price * (i.qty || 1), 0);
  const fee = store.fee ?? 5; const total = sub + fee;
  const min = store.minOrder ?? 30;
  $('ed-cbody').innerHTML = cart.map((item) => `
    <div class="ed-ci">
      <div class="ed-ci-img">${item.img ? `<img src="${item.img}" alt="">` : '≠ÉÏÚ'}</div>
      <div class="ed-ci-info">
        ...
        <div class="ed-ci-ctrl">
          <button class="ed-qb" onclick="edChangeQty(${item.id},-1)">‘Í∆</button>
...
function startSocial() {
  ...
  $('ed-social-sub').textContent = `+‹ltima venda h+Ì ${Math.floor(Math.random()*15+1)} minutos`;
}
